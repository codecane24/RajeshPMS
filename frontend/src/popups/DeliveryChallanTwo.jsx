...
export default function DeliveryChallanTwo({ onClose }) {
  return (
    <AnimatePresence>
<div className="fixed inset-0 flex items-center justify-center z-50">
  ...
  {/* Popup */}
  <motion.div
    className="relative bg-white rounded-lg shadow-lg w-full max-w-5xl max-h-[95vh] p-6 z-10 overflow-x-auto overflow-y-auto"
...
<div className="px-20 oerflow-x-auto mt-4">
  <table className="w-full   text-sm md:text-lg">
    ...
    <div className="flex items-center justify-start gap-1  mt-9 pl-4"> <input type="checkbox" className="w-4 h-4 accent-black" />
<p>I have read all the details carefully and authenticating to deliver.</p></div>
   {/* Row 3: Buttons aligned right */}
          <div className="flex justify-start gap-3 mt-4 w-full">
       
                <button onClick={() => setOpenModal("open")} className="bg-[#2D740C] rounded-md text-sm text-white px-4 py-2   hover:scale-95">
         Approval Challan
            </button>
            <div className="flex justify-start items-center w-auto">
              <button onClick={() => setOpenModal("open")} className="bg-[#ED1C24] rounded-l-md text-sm text-white px-4 py-2   hover:scale-95">
    Reject
            </button>
            <input type="text" placeholder="Why Rejected ?" className="pl-2 py-2 rounded-r-md text-sm border-[1px] border-gray-400 w-full md:w-[40vw] lg:w-[55vw] "/></div>
             <button
